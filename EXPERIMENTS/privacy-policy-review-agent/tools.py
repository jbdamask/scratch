...
def scrape_page(url: str) -> str:
    """Scrape visible text and on-page links (no following) and save to markdown file."""
    try:
        headers = {
        ...
        # On-page links (absolute, deduped)
        links = []
        seen = set()
        ...
        return (
            f"===== TEXT =====\n{text}\n\n"
            f"===== LINKS (no recursion) =====\n{links_block}"
        )
    except Exception as e:
        return f"Error scraping {url}: {e}"

@function_tool
def scrape_and_save(vendor, url: str) -> str:
    """Scrape visible text and on-page links (no following) and save to markdown file."""
    try:
        ...
        return write_markdown(vendor, url)
